...
// div for event name and location
  ...
  // var venueContentDiv = document.createElement('div');
  // eventContentDiv.setAttribute("id", "venue_content_div" + i);
  // eventContentDiv.innerHTML = eventsList[i].eventDescription;
  // thisEventBox.appendChild(eventContentDiv);
  ...
  returnToMap.addEventListener('click', function() { 
    ...
    $("#eventBoxNo0").css({'display': 'none'});  // this box left opened by the event listeners on next/last event buttons
    $("#venue_content_div").css({'display': 'none'});
    $("#results_count").children('span').text(""); // clearing the number of last viewed event from results div
    $("#earliest_result").css({'display': 'block'});
  })
  }


// APPEND RELEVANT INFO TO THE VENUE DIV, SET DIAPLAY TO BLOCK, THEN SET LISTENERS ON SEARCH BUTTON, EVENT NAME, RETURN TO MAP BUTTON, NEXT, LAST (AND CALENDAR IF ADDED BUTTON TO REMOVE THIS DIV (COULD JUST HIDE DISPLAY FOR CLICK ON EVENT NAME))
...
function eventPopUp(event) {
  ...
  $("#earliest_result").css({'display': 'none'});
